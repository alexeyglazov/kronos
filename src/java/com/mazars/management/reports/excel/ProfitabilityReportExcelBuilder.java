 ...
 * @author Glazov
 */
public class ProfitabilityReportExcelBuilder {
    ...
    public ProfitabilityReportExcelBuilder(ProfitabilityReport report) throws RowsExceededException, WriteException {
        this.report = report;
    }
    ...
    public void makeContent(List<ProfitabilityReport.Row> rows, WritableSheet sheet) throws RowsExceededException, WriteException {
        ...
        for(ProfitabilityReport.Row row : rows) {
            ...
            for(Currency currency : this.report.getCurrencies()) {
                if(row.getStandardSellingRate() != null && currency.getId().equals(row.getStandardSellingRateGroup().getCurrency().getId())) {
            ...
            for(Currency currency : this.report.getCurrencies()) {
                if(row.getStandardCost() != null && currency.getId().equals(row.getStandardCostGroup().getCurrency().getId())) {
    ...
    public void fillDividedWorkbook(List<ProfitabilityReport.Row> rows, WritableWorkbook workbook) throws RowsExceededException, WriteException {
        ...
        for(ProfitabilityReport.Row row : rows) {
            tmpClients.add(row.getProjectCode().getClient());
        }
        List<Client> clients = new LinkedList<Client>(tmpClients);
        Collections.sort(clients, new ClientComparator(ClientComparator.Mode.CODE_NAME));
        int count = 0;
