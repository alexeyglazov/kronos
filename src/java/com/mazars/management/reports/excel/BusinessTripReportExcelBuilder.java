 ...
 * @author Glazov
 */
public class BusinessTripReportExcelBuilder {
    ...
    public BusinessTripReportExcelBuilder(BusinessTripReport report) throws RowsExceededException, WriteException {
        this.report = report;
    }
    ...
    public void makeContent(List<BusinessTripReport.Row> rows, WritableSheet sheet) throws RowsExceededException, WriteException {
        ...
        columnNumber++;
        sheet.addCell(new Label(columnNumber, rowNumber, "First name (local language)", headingFormat));
        columnNumber++;
        sheet.addCell(new Label(columnNumber, rowNumber, "Last name (local language)", headingFormat));
        columnNumber++;
        sheet.addCell(new Label(columnNumber, rowNumber, "Days count", headingFormat));
        columnNumber++;
        ...
        int businessTripsColumnNumber = 6;
        for(BusinessTripReport.Row row : rows) {
            columnNumber = 0;
            ...
            columnNumber++;
            if(row.getEmployee() != null && row.getEmployee().getFirstNameLocalLanguage() != null) {
                sheet.addCell(new jxl.write.Label(columnNumber, rowNumber, row.getEmployee().getFirstNameLocalLanguage()));
            }
            columnNumber++;
            if(row.getEmployee() != null && row.getEmployee().getLastNameLocalLanguage() != null) {
                sheet.addCell(new jxl.write.Label(columnNumber, rowNumber, row.getEmployee().getLastNameLocalLanguage()));
            }
            columnNumber++;
            if(row.getBusinessTripDaysCount() != null) {
                sheet.addCell(new jxl.write.Number(columnNumber, rowNumber, row.getBusinessTripDaysCount() ));
            }
            columnNumber++;
        ...
        if(firstRowToSumNumber <= (rowNumber - 1)) {
            String formula = "SUM(" + ExcelUtils.getColumnName(businessTripsColumnNumber) + (firstRowToSumNumber + 1) + ":" + ExcelUtils.getColumnName(businessTripsColumnNumber) + (rowNumber - 1 + 1) + ")";
            sheet.addCell(new jxl.write.Formula(businessTripsColumnNumber, rowNumber, formula));
        }
   }
