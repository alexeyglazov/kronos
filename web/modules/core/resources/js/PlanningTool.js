...
PlanningTool.prototype.calculateDisplayPeriod = function() {
    ...
    this.time.displayPeriod = {
        start : getShiftedYearMonthDate(this.time.cursor, Math.floor(-0.1 * displayDays)),
        end : getShiftedYearMonthDate(this.time.cursor, 2* displayDays)
    }
}
...
PlanningTool.prototype.savePlanningItem = function(planningItem) {
    var planningItems = [];
    planningItems.push(planningItem);
    this.savePlanningItems(planningItems);
}
PlanningTool.prototype.savePlanningItems = function(planningItems) {
...
PlanningTool.prototype.show = function() {
    this.updateView();
    this.scrollTo(this.time.cursor);
}
PlanningTool.prototype.getHtml = function() {
    ...
    html += '<table>';
    html += '<tr>';
    html += '<td style="height: 20px; padding: 0px;"></td>'
    html += '<td style="height: 20px; padding: 0px;" id="' + this.htmlId + '_layoutHeader">';
        html += '<div style="width: 200px; height: 20px; overflow-x: hidden; overflow-y: scroll;" id="' + this.htmlId + '_header" class="planning"></div>';
    html += '</td>';   
    html += '</tr>';
    html += '<tr>';
    html += '<td id="' + this.htmlId + '_layoutEmployees" style="padding: 0px;">';
        html += '<div style="width: 200px; height: 300px; overflow-y: hidden;" id="' + this.htmlId + '_employees" class="planning"></div>';
    html += '</td>'
    html += '<td id="' + this.htmlId + '_layoutPlanning" style="padding: 0px;">';
        html += '<div style="width: 300px; height: 300px; overflow-x: auto; position: relative;" id="' + this.htmlId + '_planning" class="planning"></div>'
    html += '</td>';   
    html += '</tr>';
    html += '</table>';
...
PlanningTool.prototype.updateEmployeesView = function() {
    ...
    for(var eKey in this.info.employees) {
        ...
        var tmpPositionId = null;
        if(employee.employeePositionHistoryItems != null && employee.employeePositionHistoryItems.length != 0) {
            var employeePositionHistoryItem = employee.employeePositionHistoryItems[0];
            var position = this.info.getPosition(employeePositionHistoryItem.positionId);
            if(position != null) {
                tmpPositionId = position.id;
            }
        }     
        if(positionId != tmpPositionId) {
        ...
        html += '<tr id="' + trId + '" ' + (highlight ? 'class="dgHighlight4" ' : '' ) + '>';
...
PlanningTool.prototype.updateCellInfo = function() {
    ...
    if(employeePositionHistoryItem != null) {
        ...
        subdepartment = this.info.getSubdepartment(position.subdepartmentId);
    }
    ...
    html += '<tr><td><div class="comment1">Subdepartment</div>' + (subdepartment != null ? (subdepartment.officeName + ' / ' + subdepartment.departmentName + ' / ' + subdepartment.subdepartmentName) : 'No subdepartment') + '</td></tr>';
...
PlanningTool.prototype.updateEmployeeInfo = function() {
    ...
    for(var key in employee.employeePositionHistoryItems) {
        ...
        var subdepartment = this.info.getSubdepartment(position.subdepartmentId);
        html += '<tr><td>';
        ...
        html += '<div class="comment1">Subdepartment</div>';
        html += (subdepartment != null ? (subdepartment.officeName + ' / ' + subdepartment.departmentName + ' / ' + subdepartment.subdepartmentName) : 'No subdepartment') + '<br />';
...
PlanningTool.prototype.removePlanningItemOKHandle = function() {
    ...
    this.info.removePlanningItem(this.selected.planningItemId);
...
PlanningTool.prototype.initPlanningItemBatchCreation = function() {
    ...
    var options = {
        targetSubdepartments: targetSubdepartments,
        sourceSubdepartments: sourceSubdepartments,
        sourceSubdepartmentId: this.selected.subdepartmentId,
        date: this.time.cursor
    }
    options.date = this.time.cursor;
    if(this.selected.cell != null && this.selected.cell.day != null) {
        if(compareYearMonthDate(this.selected.cell.day, this.time.displayPeriod.start) >= 0 && compareYearMonthDate(this.selected.cell.day, this.time.displayPeriod.end) <= 0) {
            options.date = this.selected.cell.day;
        }
    }
    var planningItemBatchCreationForm = new PlanningItemBatchCreationForm(formData, this.htmlId + '_planningItemBatchCreationForm', this.planningItemBatchCreationOKHandle, this, options);
    planningItemBatchCreationForm.init();
...
PlanningTool.prototype.gridScrollHandle = function(event) {
    ...
    var gridWidth = $('#' + this.htmlId + '_grid').width();
    var planningWidth = $('#' + this.htmlId + '_planning').width();
    //var form = this;
    //if(left > (gridWidth - planningWidth)) {
    //    clearTimeout(this.timeout_id);
    //    this.timeout_id = setTimeout(function() {form.scrollTo(); }, 1000);
    //} else {
    //    clearTimeout(this.timeout_id);
    //}
}
PlanningTool.prototype.scrollTo = function(date) {
    var days = getDaysInRange(this.time.displayPeriod.start, date);
    $('#' + this.htmlId + '_planning').scrollLeft(days*25);
}
...
PlanningTool.prototype.planningItemMovedHandle = function(event, ui) {
    ...
    this.savePlanningItem(planningItem);
